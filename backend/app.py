...
def init_db():
    ...
    with app.app_context():
        ...
        db.executescript(
            ...
            -- ================================================================
            -- INGREDIENT TRANSFORMATION SYSTEM (molecular gastronomy)
            -- ================================================================

            -- 1. INGREDIENT PREPARATIONS (separate nutrition profiles for transformations)
            ...
            -- 2. INGREDIENT COMPOSITION (what prepared items are made from)
            ...
            -- 3. TRANSFORMATION PROCESSES (the "how" database with chemistry)
            CREATE TABLE IF NOT EXISTS transformation_processes (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            -- 4. TRANSFORMATION_RECIPES (specific how-tos linking ingredients + process)
            ...
            -- 5. EQUIPMENT CAPABILITIES (preset database)
            ...
            -- 6. FERMENTATION PROTOCOLS (specialized chemistry tracking)
            CREATE TABLE IF NOT EXISTS fermentation_protocols (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            -- 7. BOTANICAL FAMILIES (scientific diversity tracking)
            CREATE TABLE IF NOT EXISTS botanical_families (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            -- 8. INGREDIENT_BOTANICAL_CLASSIFICATION (linking table)
            CREATE TABLE IF NOT EXISTS ingredient_botanical_classification (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            -- 9. DIVERSITY_TRACKING (daily consumption log)
            CREATE TABLE IF NOT EXISTS diversity_tracking (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ingredient_id INTEGER,
                family_id INTEGER,
                consumed_date DATE,
            ...
            -- 10. DIVERSITY_SUMMARY (pre-calculated metrics)
            CREATE TABLE IF NOT EXISTS diversity_summary (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                period_type TEXT,
            ...
            -- 11. PREPARATION_CHEMISTRY_LOG (experiment tracker)
            CREATE TABLE IF NOT EXISTS preparation_chemistry_log (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            -- ====================================================================
            -- MOLECULAR GASTRONOMY SYSTEM TABLES
            -- ====================================================================

            -- 12. COOKING_METHODS (master list of cooking techniques)
            ...
            -- 13. INGREDIENT_COOKING_PROFILES (how ingredients respond to cooking methods)
            ...
            -- 14. PREP_METHOD_EFFECTS (how cutting style affects flavor)
            CREATE TABLE IF NOT EXISTS prep_method_effects (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ingredient_id INTEGER NOT NULL,
            ...
            -- ALTER TABLE statements to extend existing tables
            -- Note: SQLite doesn't support multiple ALTER COLUMN in one statement, so we do them individually
            -- These are designed to fail silently if columns already exist (SQLite behavior)

            -- Extend transformation_processes with flavor/texture tracking
            CREATE TABLE IF NOT EXISTS transformation_processes_temp AS SELECT * FROM transformation_processes;
            DROP TABLE IF EXISTS transformation_processes_temp;
            -- The above ensures the table exists before we alter it

            -- ================================================================
            -- ANTHROPOLOGICAL FOOD SYSTEM
            -- ================================================================

            -- Cultures/civilizations through history
            ...
            -- Master food catalog (every distinct food item)
            ...
            -- Foods linked to origin cultures (M2M)
            CREATE TABLE IF NOT EXISTS food_culture_origins (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            -- Foods linked to meal types (M2M, culture-specific)
            ...
            -- How a culture eats throughout the day
            CREATE TABLE IF NOT EXISTS culture_meal_patterns (
                ...
                typical_foods TEXT,
                social_context TEXT,
                class_variations TEXT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (culture_id) REFERENCES cultures(id),
                FOREIGN KEY (meal_type_id) REFERENCES meal_types(id)
            );

            -- Historically-grounded recipes
            CREATE TABLE IF NOT EXISTS anthropological_recipes (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                name TEXT NOT NULL,
                name_native TEXT,
                slug TEXT UNIQUE NOT NULL,
            ...
            -- Junction table linking recipe ingredients to foods catalog
            CREATE TABLE IF NOT EXISTS anthropological_recipe_ingredients (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                recipe_id INTEGER NOT NULL,
                food_id INTEGER,
                ingredient_name TEXT NOT NULL,
            ...
            -- Seed the 13 meal types
            INSERT OR IGNORE INTO meal_types (name, slug, description, icon) VALUES
                ('Breakfast', 'breakfast', 'Morning meal', '🌅'),
                ('Lunch', 'lunch', 'Midday meal', '☀️'),
                ('Dinner', 'dinner', 'Evening meal', '🌙'),
                ('Snack', 'snack', 'Between-meal bites', '🍿'),
                ('Dessert', 'dessert', 'Sweet course', '🍰'),
                ('Beverage', 'beverage', 'Drinks and refreshments', '🍵'),
                ('Ceremonial', 'ceremonial', 'Ritual and religious meals', '🕯️'),
                ('Medicinal', 'medicinal', 'Healing foods and remedies', '💊'),
                ('Street Food', 'street_food', 'Quick bites from vendors', '🏪'),
                ('Tea Time', 'tea_time', 'Afternoon tea and accompaniments', '🫖'),
                ('Brunch', 'brunch', 'Late morning combined meal', '🥂'),
                ('Supper', 'supper', 'Light evening meal', '🕰️'),
                ('Feast', 'feast', 'Celebratory banquet meals', '👑');

            -- ================================================================
            -- HERBALISM FRAMEWORK (extends alchemy)
            -- ================================================================

            -- Traditional medicine paradigms
            CREATE TABLE IF NOT EXISTS medicine_systems (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            -- How each herb is classified per medicine system
            CREATE TABLE IF NOT EXISTS herb_system_classifications (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            -- Which cultures used which herbs, when, for what
            CREATE TABLE IF NOT EXISTS herb_historical_uses (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                alchemy_ingredient_id INTEGER NOT NULL,
            ...
            -- Preparation methods beyond brewing
            CREATE TABLE IF NOT EXISTS herbal_preparation_methods (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
            ...
            -- Specific prep instructions per herb+method
            CREATE TABLE IF NOT EXISTS herb_preparations (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                alchemy_ingredient_id INTEGER NOT NULL,
            ...
            -- Safety database: herb-drug interactions
            ...
            -- Traditional multi-herb formulas with cultural stories
            ...
            -- Bridge: same herb in food vs. medicine across cultures
            CREATE TABLE IF NOT EXISTS herb_culinary_traditions (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                alchemy_ingredient_id INTEGER NOT NULL,
            ...
            -- Full encyclopedia entry per herb
            ...
            -- User progression for herbalism (parallel to brewing_journal)
            CREATE TABLE IF NOT EXISTS herbalism_journal (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        # Add columns to existing tables (SQLite requires individual ALTER TABLE statements)
        # These will fail silently if columns already exist
        try:
            # Extend transformation_processes
            db.execute("ALTER TABLE transformation_processes ADD COLUMN flavor_changes_json TEXT")
        except:
            pass
        try:
            db.execute("ALTER TABLE transformation_processes ADD COLUMN texture_changes_json TEXT")
        except:
            pass
        try:
            db.execute("ALTER TABLE transformation_processes ADD COLUMN enzyme_activity TEXT")
        except:
            pass
        try:
            db.execute("ALTER TABLE transformation_processes ADD COLUMN volatile_compounds TEXT")
        except:
            pass

        try:
            # Extend ingredients
            db.execute("ALTER TABLE ingredients ADD COLUMN base_flavor_profile TEXT")
        except:
            pass
        try:
            db.execute("ALTER TABLE ingredients ADD COLUMN base_texture TEXT")
        except:
            pass
        try:
            db.execute("ALTER TABLE ingredients ADD COLUMN key_flavor_compounds TEXT")
        except:
            pass
        try:
            db.execute("ALTER TABLE ingredients ADD COLUMN enzyme_systems TEXT")
        except:
            pass

        # Herbalism extensions to alchemy_ingredients
        try:
            db.execute("ALTER TABLE alchemy_ingredients ADD COLUMN has_monograph INTEGER DEFAULT 0")
        except:
            pass
        try:
            db.execute("ALTER TABLE alchemy_ingredients ADD COLUMN botanical_family_id INTEGER")
        except:
            pass
...
@app.route("/ingredient/transformations")
def ingredient_transformations():
    """Molecular Gastronomy - Ingredient transformation discovery page."""
    return render_template("ingredient_transformations.html")


@app.route("/transformations-demo")
def transformations_demo():
    """Demo page showing all existing transformations as cards."""
    db = get_db()

    # Get ingredients that have transformation data
    ingredient_ids = [38, 11]  # Diced Tomatoes and Garlic
    ingredients_data = []

    for ing_id in ingredient_ids:
        # Get ingredient info
        ingredient = db.execute("""
            SELECT id, name, base_flavor_profile, base_texture,
                   key_flavor_compounds, enzyme_systems
        ...
        # Get cooking transformations
        cooking_profiles = db.execute("""
            SELECT
                cm.method_name,
                cm.method_category,
                cm.temp_range_min_f,
                ...
                icp.flavor_before,
                icp.flavor_after,
                icp.flavor_intensity_change,
                icp.primary_reactions,
                tp.beginner_explanation,
                tp.chemistry_explanation,
                tp.expert_explanation
            FROM ingredient_cooking_profiles icp
            JOIN cooking_methods cm ON cm.id = icp.cooking_method_id
            LEFT JOIN transformation_processes tp ON tp.id = icp.process_id
            WHERE icp.ingredient_id = ?
            ORDER BY cm.method_category, cm.method_name
        """, (ing_id,)).fetchall()

        cooking_transformations = []
        for cp in cooking_profiles:
            heat_type = "Dry" if cp["method_category"] in ["dry-heat", "combination"] else "Moist"
            temp_range = f"{cp['temp_range_min_f']}-{cp['temp_range_max_f']}°F" if cp["temp_range_min_f"] else None

            # Parse flavor intensity - handle strings like "3x" or numeric values
            flavor_mult = 1.0
            if cp["flavor_intensity_change"]:
                try:
                    # If it's a string like "3x", strip the "x" and convert
                    val = str(cp["flavor_intensity_change"]).replace("x", "").strip()
                    flavor_mult = float(val)
                except:
                    flavor_mult = 1.0

            cooking_transformations.append({
                "method_name": cp["method_name"],
                "heat_type": heat_type,
                "temp_range_f": temp_range,
                "texture_changes": f"{cp['texture_before']} → {cp['texture_after']}" if cp["texture_before"] else "Varies",
                "flavor_changes": f"{cp['flavor_before']} → {cp['flavor_after']}" if cp["flavor_before"] else "Enhanced",
                "flavor_intensity_multiplier": flavor_mult,
                "chemistry_beginner": cp["beginner_explanation"],
                "chemistry_detail": cp["chemistry_explanation"],
                "chemistry_expert": cp["expert_explanation"]
            })

        # Get prep methods
        prep_methods = db.execute("""
            SELECT
                prep_method,
                surface_area_ratio,
                flavor_intensity,
                ...
                best_for
            FROM prep_method_effects
            WHERE ingredient_id = ?
            ORDER BY surface_area_ratio
        """, (ing_id,)).fetchall()
        ...
        for pm in prep_methods:
            ...
            surface_area = 1.0
            flavor_intensity = 1.0

            if pm["surface_area_ratio"]:
                try:
                    val = str(pm["surface_area_ratio"]).replace("x", "").strip()
                    surface_area = float(val)
                except:
                    surface_area = 1.0

            if pm["flavor_intensity"]:
                try:
                    val = str(pm["flavor_intensity"]).replace("x", "").strip()
                    flavor_intensity = float(val)
                except:
                    flavor_intensity = 1.0

            prep_variations.append({
                "prep_variation": pm["prep_method"],
                "surface_area_multiplier": surface_area,
                "flavor_intensity_multiplier": flavor_intensity,
                "flavor_impact": pm["compounds_released"] or "Standard flavor release",
                "texture_impact": pm["texture_distribution"] or "Standard texture",
                "chemistry_explanation": pm["enzyme_exposure"] or ""
            })

        # Get fermentation (empty for now)
        fermentation_transformations = []
        ...
        ingredients_data.append({
            ...
            "cooking_transformations": cooking_transformations,
            "prep_methods": prep_variations,
            "fermentation_transformations": fermentation_transformations,
            "counts": {
                "cooking": len(cooking_transformations),
                "prep": len(prep_variations),
                "fermentation": len(fermentation_transformations)
            }
        })

    return render_template("transformations_demo.html", ingredients=ingredients_data)


@app.route("/crafting")
def crafting_system():
    """Unified crafting system - Ingredient → Process → Dish flow."""
    return render_template("crafting_system.html")


@app.route("/process/detail")
def process_detail():
    """Process detail page - shows all recipes using this process."""
...
@app.route("/cook")
def cook_view():
    """Cooking mode default page - redirect to cooking deck or first available recipe."""
    db = get_db()

    # Try to get first recipe from cooking deck
    deck_recipe = db.execute("""
    ...
    if deck_recipe:
        # Cook from cooking deck
        if deck_recipe['recipe_source'] == 'mealdb':
            return redirect(f"/cook/mealdb/{deck_recipe['recipe_id']}")
        else:
            return redirect(f"/cook/{deck_recipe['recipe_id']}")

    # Otherwise get first local recipe
    recipe = db.execute("SELECT id FROM recipes LIMIT 1").fetchone()
    if recipe:
        return redirect(f"/cook/{recipe['id']}")

    # No recipes found - redirect to swipe to discover some
    return redirect("/swipe")


@app.route("/swipe")
def swipe_view():
    """Recipe swipe/review page - redirect to default dinner deck."""
    # Default to dinner deck (meal_type dinner)
    return redirect("/swipe/dinner")
...
@app.route("/alchemy")
def alchemy_view():
    """Recipe creation lab - CARD BASED UI."""
    return render_template("alchemy_cards.html")
...
@app.route("/anthropological")
def anthropological_view():
    """Culture Kitchen - anthropological food explorer."""
    return render_template("anthropological.html")


@app.route("/herbalism")
def herbalism_view():
    """Herbalism grimoire - traditional medicine explorer."""
    return render_template("herbalism.html")
...
def find_or_create_ingredient(cursor, name, category=None, aldi_section=None, default_unit=None):
    ...
    cursor = db.execute("SELECT id FROM ingredients WHERE LOWER(name) = LOWER(?)", [name])
...
def api_recipe_steps(recipe_id):
    ...
    steps = db.execute(
        """
        SELECT cs.step_number, cs.title, cs.instruction, cs.duration_min, cs.tips, cs.timer_needed, cs.step_type,
               cs.process_id,
               tp.process_name,
               tp.process_type
        FROM cooking_steps cs
        LEFT JOIN transformation_processes tp ON cs.process_id = tp.id
        WHERE cs.recipe_id = ?
    ...
    for step in steps:
        ...
        # Add process information if present
        if step_dict.get("process_id"):
            step_dict["process"] = {
                "id": step_dict["process_id"],
                "name": step_dict["process_name"],
                "type": step_dict["process_type"]
            }
        else:
            step_dict["process"] = None

        # Clean up - remove redundant fields
        step_dict.pop("process_id", None)
        step_dict.pop("process_name", None)
        step_dict.pop("process_type", None)
...
# ============================================================================
# API ROUTES - SMART SCALE INTEGRATION
# ============================================================================


@app.route("/api/scale/containers", methods=["GET", "POST"])
def api_scale_containers():
    db = get_db()
    """Get all scale containers or create new one."""
    db = get_db()
    if request.method == "GET":
        cursor = db.execute("""
            SELECT id, name, tare_weight_g, color_hex, icon_emoji, notes, created_at
            FROM scale_containers
            ORDER BY name
        """)
        containers = []
        ...
        data = request.json
        cursor = db.execute("""
            INSERT INTO scale_containers (name, tare_weight_g, color_hex, icon_emoji, notes)
            VALUES (?, ?, ?, ?, ?)
        """, (
            data["name"],
            data["tare_weight_g"],
        ...
        db.commit()
        return jsonify({"success": True, "container_id": cursor.lastrowid})


@app.route("/api/scale/measure", methods=["POST"])
def api_scale_measure():
    db = get_db()
    """Log a scale measurement."""
    db = get_db()
    data = request.json
    cursor = db.execute("""
        INSERT INTO scale_measurements
        (product_id, pantry_inventory_id, gross_weight_g, tare_weight_g, net_weight_g,
         container_id, measurement_type, recipe_id, notes)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data.get("product_id"),
        data.get("pantry_inventory_id"),
        data["gross_weight_g"],
        data.get("tare_weight_g", 0),
        data["net_weight_g"],
    ...
    db.commit()
    return jsonify({"success": True, "measurement_id": cursor.lastrowid})


@app.route("/api/scale/measurements")
def api_scale_measurements():
    ...
    query = """
        SELECT sm.id, sm.gross_weight_g, sm.net_weight_g, sc.name as container_name, sm.measured_at
        FROM scale_measurements sm
        LEFT JOIN scale_containers sc ON sm.container_id = sc.id
    """
    params = []
    ...
    query += " ORDER BY sm.measured_at DESC LIMIT ?"
    params.append(limit)
...
@app.route("/api/pantry/inventory/<int:inventory_id>/weigh", methods=["POST"])
def api_weigh_pantry_item(inventory_id):
    """Update pantry inventory with scale measurement."""
    db = get_db()
    data = request.json
    net_weight = data["net_weight_g"]

    cursor = db.execute("""
        UPDATE pantry_inventory
        SET current_weight_g = ?, measured_by_scale = 1, updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
    """, (net_weight, inventory_id))

    # Optionally save tare weight
    if data.get("save_tare") and data.get("tare_weight_g"):
        cursor = db.execute("""
            UPDATE pantry_inventory
            SET tare_weight_g = ?
            WHERE id = ?
        """, (data["tare_weight_g"], inventory_id))

    db.commit()
    return jsonify({"success": True, "new_weight": net_weight})


@app.route("/api/scale/infusions", methods=["GET", "POST"])
def api_scale_infusions():
    """Get infusions or create new one."""
    db = get_db()
    if request.method == "GET":
        status = request.args.get("status", "active")
        cursor = db.execute("""
            SELECT id, name, type, start_date, target_duration_days,
                   initial_weight_g, current_weight_g, status, created_at
            FROM infusion_tracking
            WHERE status = ?
        ...
        for row in cursor.fetchall():
            infusions.append({
                ...
                "type": row[2],
                "start_date": row[3],
                "target_duration_days": row[4],
                "initial_weight_g": row[5],
                "current_weight_g": row[6],
                "status": row[7],
                "created_at": row[8],
            })
        return jsonify(infusions)

    elif request.method == "POST":
        data = request.json
        cursor = db.execute("""
            INSERT INTO infusion_tracking
            (name, type, start_date, target_duration_days, initial_weight_g, reminder_interval_days)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (
            data["name"],
            data["type"],
            data["start_date"],
            data["target_duration_days"],
            data["initial_weight_g"],
            data.get("reminder_interval_days", 3),
        ))
        db.commit()
        return jsonify({"success": True, "infusion_id": cursor.lastrowid})


@app.route("/api/scale/infusions/<int:infusion_id>/checkin", methods=["POST"])
def api_infusion_checkin(infusion_id):
    """Log infusion check-in (weight measurement)."""
    db = get_db()
    data = request.json

    # Calculate day number
    cursor = db.execute("SELECT start_date FROM infusion_tracking WHERE id = ?", (infusion_id,))
    row = cursor.fetchone()
    if not row:
        return jsonify({"error": "Infusion not found"}), 404

    start_date = datetime.strptime(row[0], "%Y-%m-%d")
    day_number = (datetime.now() - start_date).days

    # Insert check-in
    cursor = db.execute("""
        INSERT INTO infusion_check_ins (infusion_id, day_number, weight_g, notes)
        VALUES (?, ?, ?, ?)
    """, (infusion_id, day_number, data["weight_g"], data.get("notes")))

    # Update current weight
    cursor = db.execute("""
        UPDATE infusion_tracking
        SET current_weight_g = ?
        WHERE id = ?
    """, (data["weight_g"], infusion_id))

    db.commit()
    return jsonify({"success": True})


@app.route("/api/scale/infusions/<int:infusion_id>/checkins")
def api_infusion_checkins(infusion_id):
    """Get infusion check-in history."""
    db = get_db()
    cursor = db.execute("""
        SELECT id, day_number, weight_g, notes, logged_at
        FROM infusion_check_ins
        WHERE infusion_id = ?
        ORDER BY day_number
    """, (infusion_id,))
    checkins = []
...
@app.route("/api/scale/brew-logs", methods=["GET", "POST"])
def api_brew_logs():
    """Get brew logs or create new one."""
    db = get_db()
    if request.method == "GET":
        method = request.args.get("method")
        limit = request.args.get("limit", 20, type=int)

        query = "SELECT * FROM brew_logs"
        params = []

        if method:
            query += " WHERE brew_method = ?"
        ...
        for row in cursor.fetchall():
            logs.append({
                "id": row[0],
                "brew_method": row[1],
                "coffee_weight_g": row[2],
                "water_weight_g": row[3],
                "ratio": row[4],
                "grind_size": row[5],
                "brew_time_seconds": row[6],
                "water_temp_c": row[7],
                "rating": row[8],
                "notes": row[9],
                "created_at": row[10],
        ...
        if not ratio and data["coffee_weight_g"] > 0:
            ratio = data["water_weight_g"] / data["coffee_weight_g"]

        cursor = db.execute("""
            INSERT INTO brew_logs
            (brew_method, coffee_weight_g, water_weight_g, ratio, grind_size,
             brew_time_seconds, water_temp_c, rating, notes)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            data["brew_method"],
            ...
            ratio,
            data.get("grind_size"),
            data.get("brew_time_seconds"),
            data.get("water_temp_c"),
            data.get("rating"),
            data.get("notes"),
        ))
        db.commit()
        return jsonify({"success": True, "brew_id": cursor.lastrowid})
...
# NOTE: /shopping route already defined earlier at line ~1620
...
def get_meal_plan(plan_id):
    ...
    # Get plan details
    cursor = db.execute("SELECT * FROM meal_plans WHERE id = ?", (plan_id,))
...
def init_alchemy_data():
    ...
    with app.app_context():
        ...
        effects_count = cursor = db.execute("SELECT COUNT(*) FROM potion_effects").fetchone()[0]

        # Check if alchemy_ingredients already populated
        ingredients_count = cursor = db.execute("SELECT COUNT(*) FROM alchemy_ingredients").fetchone()[0]
        ...
        cursor = db.execute("INSERT OR IGNORE INTO brewing_journal (id) VALUES (1)")
...
# Alchemy API Routes
# NOTE: /alchemy route already defined earlier at line ~1736
...
def search_ingredients():
    ...
    Example: /api/ingredients/search?q=tomato
    """
...
@app.route("/api/ingredients/<int:ingredient_id>/transformations")
def get_ingredient_transformations(ingredient_id):
    """
    Get all possible transformations for an ingredient.

    Returns what you can make with this ingredient - the crafting/discovery system.
    UPDATED: Now includes cooking methods (sauté, roast, grill, etc.) AND fermentation transformations.

    Example: /api/ingredients/29/transformations → Milk can become:
    - Yogurt (needs: starter culture, equipment: instant pot)
    - Kefir (needs: kefir grains, equipment: jar)
    - Butter (needs: cream separation, equipment: mixer)
    - Cheese (needs: rennet, equipment: pot)

    Example: /api/ingredients/42/transformations → Tomatoes can be:
    - Sautéed (flavor: 1x → 3x, texture: firm → soft)
    - Roasted (flavor: 1x → 5x, texture: firm → jammy)
    - Grilled (flavor: 1x → 5x, texture: charred)
    - Sliced (prep method, mild intensity)
    - Crushed (prep method, intense intensity)
    """
    db = get_db()

    # Get ingredient info (including new molecular gastronomy fields)
    ingredient = db.execute("""
        SELECT id, name, base_flavor_profile, base_texture,
               key_flavor_compounds, enzyme_systems
        FROM ingredients WHERE id = ?
    ...
    # Get all transformations where this is the base ingredient
    transformations = db.execute("""
        SELECT
            tr.id,
            tr.recipe_name,
            tr.yield_percent,
            tr.instructions_json,
            tr.quality_metrics,
            ip.preparation_name as output_name,
            ip.calories_per_100g as output_calories,
            ip.protein_per_100g as output_protein,
            tp.id as process_id,
            tp.process_name,
            tp.process_type,
            tp.temp_optimal_f,
            tp.duration_min_hours,
            tp.duration_max_hours,
            tp.beginner_explanation,
            tp.chemistry_explanation,
            tp.expert_explanation,
            kt.name as equipment_name,
            kt.category as equipment_category
        FROM transformation_recipes tr
        LEFT JOIN ingredient_preparations ip ON tr.output_preparation_id = ip.id
        LEFT JOIN transformation_processes tp ON tr.process_id = tp.id
        LEFT JOIN kitchen_tools kt ON tr.equipment_id = kt.id
        WHERE tr.base_ingredient_id = ?
        ORDER BY tr.recipe_name
    ...
    for t in transformations:
        ...
        import json
        try:
            instructions = json.loads(t["instructions_json"]) if t["instructions_json"] else {}
        except:
            instructions = {}
        ...
        result.append({
            ...
            "output": {
                "name": t["output_name"],
                "calories_per_100g": t["output_calories"],
                "protein_per_100g": t["output_protein"]
            },
            "process": {
                "id": t["process_id"],
                "name": t["process_name"],
                "type": t["process_type"],
                "temp_f": t["temp_optimal_f"],
                "duration_hours": {
                    "min": t["duration_min_hours"],
                    "max": t["duration_max_hours"]
                },
                "explanation": {
                    "beginner": t["beginner_explanation"],
                    "chemistry": t["chemistry_explanation"],
                    "expert": t["expert_explanation"]
                }
            },
            "equipment": {
            ...
            "yield_percent": t["yield_percent"],
            "quality_metrics": t["quality_metrics"]
        })

    # Get cooking method transformations
    cooking_profiles = db.execute("""
        SELECT
            cm.id as method_id,
            cm.method_name,
            ...
            cm.temp_range_min_f,
            cm.temp_range_max_f,
            cm.typical_duration_min,
            cm.typical_duration_max,
            cm.equipment_needed,
            cm.description,
            icp.flavor_before,
            icp.flavor_after,
            icp.flavor_compounds_formed,
            icp.flavor_intensity_change,
            icp.texture_before,
            icp.texture_after,
            icp.water_activity_change,
            icp.cell_structure_change,
            icp.primary_reactions,
            icp.enzymes_activated,
            icp.enzymes_deactivated,
            icp.optimal_temp_f,
            icp.optimal_duration_min,
            icp.recommended_technique,
            icp.scientific_notes,
            icp.chef_tips,
            tp.beginner_explanation,
            tp.chemistry_explanation,
            tp.expert_explanation
    ...
    for cp in cooking_profiles:
        cooking_transformations.append({
            ...
            "category": cp["method_category"],
            "temp_range": f"{cp['temp_range_min_f']}-{cp['temp_range_max_f']}°F" if cp["temp_range_min_f"] else None,
            "duration": f"{cp['typical_duration_min']}-{cp['typical_duration_max']} min" if cp["typical_duration_min"] else None,
            "flavor_changes": {
                "before": cp["flavor_before"],
                "after": cp["flavor_after"],
                "intensity": cp["flavor_intensity_change"],
                "compounds": cp["flavor_compounds_formed"]
            },
            "texture_changes": {
                "before": cp["texture_before"],
                "after": cp["texture_after"],
                "water_activity": cp["water_activity_change"],
                "cell_structure": cp["cell_structure_change"]
            },
            "chemistry": {
                "beginner": cp["beginner_explanation"],
                "chemistry": cp["chemistry_explanation"],
                "expert": cp["expert_explanation"],
                "reactions": cp["primary_reactions"],
                "enzymes_activated": cp["enzymes_activated"],
                "enzymes_deactivated": cp["enzymes_deactivated"]
            },
            "optimal_conditions": {
                "temp": cp["optimal_temp_f"],
                "duration": cp["optimal_duration_min"],
                "technique": cp["recommended_technique"]
            },
            "notes": {
                "scientific": cp["scientific_notes"],
                "chef_tips": cp["chef_tips"]
            }
        })

    # Get prep method variations
    prep_methods = db.execute("""
        SELECT
            prep_method,
            surface_area_ratio,
    ...
    for pm in prep_methods:
        prep_variations.append({
            "method": pm["prep_method"],
            "surface_area": pm["surface_area_ratio"],
            "flavor_intensity": pm["flavor_intensity"],
            "enzyme_exposure": pm["enzyme_exposure"],
            "compounds_released": pm["compounds_released"],
            "cooking_time_impact": pm["cooking_time_impact"],
            "texture_distribution": pm["texture_distribution"],
            "best_for": pm["best_for"],
            "example_dishes": pm["example_dishes"]
        })
    ...
    return jsonify({
        "ingredient": {
            "id": ingredient["id"],
            "name": ingredient["name"],
            "base_flavor": ingredient["base_flavor_profile"],
            "base_texture": ingredient["base_texture"],
            "key_compounds": ingredient["key_flavor_compounds"],
            "enzyme_systems": ingredient["enzyme_systems"]
        },
        "cooking_transformations": cooking_transformations,
        "prep_methods": prep_variations,
        "fermentation_transformations": result,
        "counts": {
            "cooking": len(cooking_transformations),
            "prep": len(prep_variations),
            "fermentation": len(result)
        }
    })


@app.route("/api/cooking-methods")
def get_cooking_methods():
    """
    Get list of all cooking methods for filtering UI.

    Returns all cooking methods grouped by category (dry-heat, moist-heat, combination, raw-prep).
    Used for filter bar in ingredient transformations UI.

    Example: /api/cooking-methods → Returns all 19 cooking methods
    """
    db = get_db()
    ...
    for m in methods:
        method_data = {
            'id': m['id'],
            'name': m['method_name'],
            'category': m['method_category'],
            'temp_range': {
                'min': m['temp_range_min_f'],
                'max': m['temp_range_max_f']
            } if m['temp_range_min_f'] else None,
            'duration': {
                'min': m['typical_duration_min'],
                'max': m['typical_duration_max']
            } if m['typical_duration_min'] else None,
            'heat_transfer': m['heat_transfer_type'],
            'equipment': m['equipment_needed'],
            'description': m['description'],
            'common_applications': m['common_applications']
        }
        result[m['method_category']].append(method_data)

    return jsonify({
        'methods_by_category': result,
        'total_methods': len(methods),
        'categories': {
            'dry-heat': len(result['dry-heat']),
            'moist-heat': len(result['moist-heat']),
            'combination': len(result['combination']),
            'raw-prep': len(result['raw-prep'])
        }
    })
...
def get_process_detail(process_id):
    """
    Get process details and all recipes/transformations that use it.

    Example: /api/process/1 → Lactic Acid Fermentation
    Shows: Yogurt, Kefir, Sourdough, Kimchi, Pickles (all use this process)
    """
    db = get_db()

    # Get process info
    process = db.execute("""
        SELECT * FROM transformation_processes WHERE id = ?
    """, (process_id,)).fetchone()

    if not process:
        return jsonify({"error": "Process not found"}), 404

    # Get all transformation recipes using this process
    transformations = db.execute("""
        SELECT
            tr.id,
            tr.recipe_name,
            ...
            i.name as base_ingredient_name,
            ip.preparation_name as output_name,
            kt.name as equipment_name
        FROM transformation_recipes tr
        LEFT JOIN ingredients i ON tr.base_ingredient_id = i.id
        LEFT JOIN ingredient_preparations ip ON tr.output_preparation_id = ip.id
        LEFT JOIN kitchen_tools kt ON tr.equipment_id = kt.id
        WHERE tr.process_id = ?
        ORDER BY tr.recipe_name
    ...
    # Get fermentation protocols using this process (if applicable)
    fermentation_protocols = db.execute("""
        SELECT
            fp.protocol_name,
            fp.temp_optimal_f,
            fp.ph_target,
            fp.duration_days,
            fp.microorganism_species,
            i.name as base_ingredient_name
        FROM fermentation_protocols fp
        LEFT JOIN ingredients i ON fp.base_ingredient_id = i.id
        WHERE fp.protocol_name LIKE ?
    """, (f"%{process['process_name']}%",)).fetchall()

    # Get cooking steps from regular recipes that use this process
    recipe_steps = db.execute("""
        SELECT DISTINCT
            r.id as recipe_id,
        ...
        WHERE cs.process_id = ?
        ORDER BY r.name, cs.step_number
    """, (process_id,)).fetchall()
    ...
    return jsonify({
        "process": {
            ...
            "temperature": {
                "min_f": process["temp_min_f"],
                "max_f": process["temp_max_f"],
                "optimal_f": process["temp_optimal_f"]
            },
            "duration": {
                "min_hours": process["duration_min_hours"],
                "max_hours": process["duration_max_hours"]
            },
            "ph_change": {
                "start": process["ph_start"],
                "end": process["ph_end"]
            },
            "required_equipment": process["required_equipment_category"],
            "chemical_reactions": process["chemical_reactions"],
            "microorganisms": process["microorganisms"],
            "explanations": {
                "beginner": process["beginner_explanation"],
                "chemistry": process["chemistry_explanation"],
                "expert": process["expert_explanation"]
            }
        },
        "used_in_transformations": [
            {
                ...
                "name": t["recipe_name"],
                "base_ingredient": {
                    "id": t["base_ingredient_id"],
                    "name": t["base_ingredient_name"]
                },
                "output": t["output_name"],
        ...
        "fermentation_protocols": [
            {
                ...
                "base_ingredient": f["base_ingredient_name"],
                "microorganisms": f["microorganism_species"],
                "temp_f": f["temp_optimal_f"],
                "ph_target": f["ph_target"],
                "duration_days": f["duration_days"]
            } for f in fermentation_protocols
        ],
        "used_in_recipes": [
            {
        ...
        "total_uses": len(transformations) + len(fermentation_protocols) + len(recipe_steps)
    })
...
# ============================================================================
# TESTING CHECKLIST (Web Form for User Feedback)
# ============================================================================

@app.route("/testing/checklist")
def testing_checklist():
    """Display web-based testing checklist form."""
    return render_template("testing_checklist.html")


@app.route("/api/testing/submit-feedback", methods=["POST"])
def submit_testing_feedback():
    """Save user testing feedback to JSON file."""
    data = request.json
    ...
    feedback_file = os.path.join(os.path.dirname(__file__), "testing_feedback.json")
...
@app.route("/api/testing/feedback-results")
def get_testing_feedback():
    """Retrieve submitted testing feedback for Claude to read."""
    feedback_file = os.path.join(os.path.dirname(__file__), "testing_feedback.json")

    if not os.path.exists(feedback_file):
        return jsonify({"error": "No feedback submitted yet"}), 404
...
# ============================================================================
# ANTHROPOLOGICAL FOOD SYSTEM API
# ============================================================================
...
def api_cultures():
    """List/filter cultures."""
    db = get_db()
    region = request.args.get("region")
    culture_type = request.args.get("type")
    era = request.args.get("era")
    q = request.args.get("q")

    query = "SELECT c.*, (SELECT COUNT(*) FROM food_culture_origins WHERE culture_id = c.id) as food_count FROM cultures c WHERE 1=1"
    params = []
    ...
    if era:
        try:
            year = int(era)
            query += " AND (c.era_start_year IS NULL OR c.era_start_year <= ?) AND (c.era_end_year IS NULL OR c.era_end_year >= ?)"
            params.extend([year, year])
        except ValueError:
            pass

    query += " ORDER BY c.name"
    cultures = db.execute(query, params).fetchall()
    return jsonify([dict(c) for c in cultures])


@app.route("/api/cultures/<slug>")
def api_culture_detail(slug):
    """Culture with eras and food counts."""
    db = get_db()
    culture = db.execute("SELECT * FROM cultures WHERE slug = ?", (slug,)).fetchone()
    if not culture:
        return jsonify({"error": "Culture not found"}), 404

    eras = db.execute("SELECT * FROM culture_eras WHERE culture_id = ? ORDER BY start_year", (culture["id"],)).fetchall()
    food_count = db.execute("SELECT COUNT(*) as cnt FROM food_culture_origins WHERE culture_id = ?", (culture["id"],)).fetchone()["cnt"]
    recipe_count = db.execute("SELECT COUNT(*) as cnt FROM anthropological_recipes WHERE culture_id = ?", (culture["id"],)).fetchone()["cnt"]

    result = dict(culture)
    result["eras"] = [dict(e) for e in eras]
    result["food_count"] = food_count
    result["recipe_count"] = recipe_count
    return jsonify(result)


@app.route("/api/cultures/<slug>/foods")
def api_culture_foods(slug):
    """Foods from this culture."""
    db = get_db()
    culture = db.execute("SELECT id FROM cultures WHERE slug = ?", (slug,)).fetchone()
    if not culture:
        return jsonify({"error": "Culture not found"}), 404
    ...
    foods = db.execute("""
        SELECT f.*, fco.origin_type, fco.significance
        FROM foods f
        JOIN food_culture_origins fco ON f.id = fco.food_id
        WHERE fco.culture_id = ?
        ORDER BY f.name
        LIMIT ? OFFSET ?
    """, (culture["id"], per_page, offset)).fetchall()
    return jsonify([dict(f) for f in foods])


@app.route("/api/cultures/<slug>/recipes")
def api_culture_recipes(slug):
    """Recipes from this culture."""
    db = get_db()
    culture = db.execute("SELECT id FROM cultures WHERE slug = ?", (slug,)).fetchone()
    if not culture:
        return jsonify({"error": "Culture not found"}), 404
    ...
    recipes = db.execute("""
        SELECT ar.*, ce.era_name
        FROM anthropological_recipes ar
        LEFT JOIN culture_eras ce ON ar.culture_era_id = ce.id
        WHERE ar.culture_id = ?
        ORDER BY ar.approximate_year
    """, (culture["id"],)).fetchall()
    return jsonify([dict(r) for r in recipes])


@app.route("/api/cultures/<slug>/meal-patterns")
def api_culture_meal_patterns(slug):
    """How this culture eats."""
    db = get_db()
    culture = db.execute("SELECT id, name FROM cultures WHERE slug = ?", (slug,)).fetchone()
    if not culture:
        return jsonify({"error": "Culture not found"}), 404

    patterns = db.execute("""
        SELECT cmp.*, mt.name as meal_type_name, mt.icon as meal_type_icon
        FROM culture_meal_patterns cmp
        JOIN meal_types mt ON cmp.meal_type_id = mt.id
        WHERE cmp.culture_id = ?
        ORDER BY mt.id
    """, (culture["id"],)).fetchall()
    return jsonify({"culture": dict(culture), "patterns": [dict(p) for p in patterns]})


@app.route("/api/foods")
def api_foods():
    """Browse all foods (paginated, filterable)."""
    db = get_db()
    page = int(request.args.get("page", 1))
    per_page = int(request.args.get("per_page", 50))
    offset = (page - 1) * per_page
    ...
    if category:
        ...
        params.append(category)
    if q:
        query += " AND (name LIKE ? OR name_native LIKE ?)"
        params.extend([f"%{q}%", f"%{q}%"])

    total = db.execute(query.replace("SELECT *", "SELECT COUNT(*) as cnt"), params).fetchone()["cnt"]
    query += " ORDER BY name LIMIT ? OFFSET ?"
    params.extend([per_page, offset])

    foods = db.execute(query, params).fetchall()
    return jsonify({"foods": [dict(f) for f in foods], "total": total, "page": page, "per_page": per_page})


@app.route("/api/foods/<slug>")
def api_food_detail_by_slug(slug):
    """Single food with cultural links (by slug)."""
    db = get_db()
    ...
    cultures = db.execute("""
        SELECT c.name, c.slug, c.region, fco.origin_type, fco.significance
        FROM food_culture_origins fco
        JOIN cultures c ON fco.culture_id = c.id
        WHERE fco.food_id = ?
    """, (food["id"],)).fetchall()

    meal_types = db.execute("""
        SELECT mt.name, mt.icon
    ...
    tags = {}
    for t in tags_rows:
        tags.setdefault(t["tag_category"], []).append({
            "value": t["tag_value"],
    ...
    result = dict(food)
    result["cultures"] = [dict(c) for c in cultures]
    result["meal_types"] = [dict(m) for m in meal_types]
    result["tags"] = tags
    return jsonify(result)


@app.route("/api/foods/quick-search")
def api_foods_quick_search():
    """Quick name-only search for autocomplete/typeahead."""
    db = get_db()
    q = request.args.get("q", "")
    if len(q) < 2:
        return jsonify([])
    ...
    foods = db.execute("""
        SELECT f.*, GROUP_CONCAT(c.name) as culture_names
        FROM foods f
        LEFT JOIN food_culture_origins fco ON f.id = fco.food_id
        LEFT JOIN cultures c ON fco.culture_id = c.id
        WHERE f.name LIKE ? OR f.name_native LIKE ?
        GROUP BY f.id
        ORDER BY f.name
        LIMIT 50
    """, (f"%{q}%", f"%{q}%")).fetchall()
...
@app.route("/api/foods/by-meal-type/<meal_type>")
def api_foods_by_meal_type(meal_type):
    """Foods by meal type."""
    db = get_db()
    mt = db.execute("SELECT id FROM meal_types WHERE slug = ?", (meal_type,)).fetchone()
    if not mt:
        return jsonify({"error": "Meal type not found"}), 404

    foods = db.execute("""
        SELECT f.*
        FROM foods f
        JOIN food_meal_types fmt ON f.id = fmt.food_id
        WHERE fmt.meal_type_id = ?
        ORDER BY f.name
    """, (mt["id"],)).fetchall()
    return jsonify([dict(f) for f in foods])


@app.route("/api/anthropological-recipes")
def api_anthro_recipes():
    """Browse anthropological recipes."""
    db = get_db()
    page = int(request.args.get("page", 1))
    per_page = int(request.args.get("per_page", 50))
    offset = (page - 1) * per_page
    ...
    query = """
        SELECT ar.*, c.name as culture_name, c.slug as culture_slug, ce.era_name
        FROM anthropological_recipes ar
        JOIN cultures c ON ar.culture_id = c.id
        LEFT JOIN culture_eras ce ON ar.culture_era_id = ce.id
        WHERE 1=1
    """
    params = []
    ...
    if meal_type:
        query += " AND ar.meal_type = ?"
        params.append(meal_type)
    if era:
        try:
            year = int(era)
            query += " AND (ar.approximate_year IS NULL OR ABS(ar.approximate_year - ?) < 200)"
            params.append(year)
        except ValueError:
            pass

    query += " ORDER BY ar.name LIMIT ? OFFSET ?"
    params.extend([per_page, offset])
...
@app.route("/api/anthropological-recipes/<slug>")
def api_anthro_recipe_detail(slug):
    """Full recipe with context."""
    db = get_db()
    recipe = db.execute("""
        SELECT ar.*, c.name as culture_name, c.slug as culture_slug,
               c.region as culture_region, ce.era_name
        FROM anthropological_recipes ar
        JOIN cultures c ON ar.culture_id = c.id
        LEFT JOIN culture_eras ce ON ar.culture_era_id = ce.id
        WHERE ar.slug = ?
    ...
    ingredients = db.execute("""
        SELECT ari.*, f.name as food_name, f.slug as food_slug
        FROM anthropological_recipe_ingredients ari
        LEFT JOIN foods f ON ari.food_id = f.id
        WHERE ari.recipe_id = ?
    """, (recipe["id"],)).fetchall()

    result = dict(recipe)
    result["ingredients_list"] = [dict(i) for i in ingredients]
    return jsonify(result)


@app.route("/api/anthropological-recipes/random")
def api_anthro_recipe_random():
    """Random recipe from any culture/era."""
    db = get_db()
    recipe = db.execute("""
        SELECT ar.*, c.name as culture_name, c.slug as culture_slug, ce.era_name
        FROM anthropological_recipes ar
...
@app.route("/api/anthropological-recipes/explore")
def api_anthro_recipe_explore():
    """Filter by culture, era, meal type - exploration endpoint."""
    db = get_db()
    culture_slug = request.args.get("culture")
    era_id = request.args.get("era_id")
    meal_type = request.args.get("meal_type")
    social_class = request.args.get("social_class")
    ...
    if culture_slug:
        query += " AND c.slug = ?"
        params.append(culture_slug)
    if era_id:
        query += " AND ar.culture_era_id = ?"
        params.append(int(era_id))
    if meal_type:
        query += " AND ar.meal_type = ?"
        params.append(meal_type)
    if social_class:
        query += " AND ar.social_class = ?"
        params.append(social_class)
    if authenticity:
        query += " AND ar.authenticity_level = ?"
        params.append(authenticity)

    query += " ORDER BY ar.approximate_year, ar.name"
    recipes = db.execute(query, params).fetchall()
    return jsonify([dict(r) for r in recipes])
...
def api_meal_types():
    """All meal types."""
    db = get_db()
    types = db.execute("SELECT * FROM meal_types ORDER BY id").fetchall()
    return jsonify([dict(t) for t in types])


@app.route("/api/meal-patterns/<culture_slug>")
def api_meal_patterns(culture_slug):
    """Culture's daily eating patterns."""
    db = get_db()
    culture = db.execute("SELECT id, name FROM cultures WHERE slug = ?", (culture_slug,)).fetchone()
    if not culture:
        return jsonify({"error": "Culture not found"}), 404
...
@app.route("/api/meal-patterns/compare")
def api_meal_patterns_compare():
    """Compare meal patterns across cultures."""
    db = get_db()
    culture_slugs = request.args.get("cultures", "").split(",")
    if not culture_slugs or culture_slugs == [""]:
        return jsonify({"error": "Provide cultures parameter (comma-separated slugs)"}), 400

    result = {}
    for slug in culture_slugs:
        slug = slug.strip()
        culture = db.execute("SELECT id, name FROM cultures WHERE slug = ?", (slug,)).fetchone()
        if not culture:
            continue
        patterns = db.execute("""
            ...
            ORDER BY id
        """, (culture["id"],)).fetchall()
        result[slug] = {"culture": dict(culture), "patterns": [dict(p) for p in patterns]}
...
HERBALISM_LEVELS = [
    ...
    (300, "Hedge Herbalist"),
    (600, "Apothecary"),
    (1000, "Master Herbalist"),
    (1500, "Wise One"),
    (2500, "Shaman of the Green"),
]


def get_herbalism_title(xp):
    """Get title for given XP amount."""
    title = "Herb Gatherer"
    level = 1
    for i, (threshold, t) in enumerate(HERBALISM_LEVELS):
        if xp >= threshold:
            title = t
            level = i + 1
    return level, title
...
def get_or_create_herbalism_journal(db):
    """Get or create the herbalism journal."""
    journal = db.execute("SELECT * FROM herbalism_journal WHERE id = 1").fetchone()
    if not journal:
        db.execute("INSERT INTO herbalism_journal (id) VALUES (1)")
        db.commit()
        journal = db.execute("SELECT * FROM herbalism_journal WHERE id = 1").fetchone()
    return journal


@app.route("/api/herbalism/systems")
def api_herbalism_systems():
    """All medicine traditions."""
    db = get_db()
    systems = db.execute("SELECT * FROM medicine_systems ORDER BY system_name").fetchall()
    result = []
    for s in systems:
        herb_count = db.execute("SELECT COUNT(*) as cnt FROM herb_system_classifications WHERE medicine_system_id = ?", (s["id"],)).fetchone()["cnt"]
        d = dict(s)
        d["herb_count"] = herb_count
        result.append(d)
    return jsonify(result)


@app.route("/api/herbalism/systems/<code>")
def api_herbalism_system_detail(code):
    """System with its herbs."""
    db = get_db()
    system = db.execute("SELECT * FROM medicine_systems WHERE system_code = ?", (code,)).fetchone()
    if not system:
        return jsonify({"error": "Medicine system not found"}), 404

    herbs = db.execute("""
        SELECT ai.id, ai.name, ai.display_name, ai.icon, ai.category,
               ai.scientific_name, ai.tcm_temperature,
               hsc.temperature_quality, hsc.taste_qualities,
               hsc.therapeutic_actions, hsc.traditional_indications,
               hsc.dosage_traditional
        FROM herb_system_classifications hsc
        JOIN alchemy_ingredients ai ON hsc.alchemy_ingredient_id = ai.id
        WHERE hsc.medicine_system_id = ?
...
@app.route("/api/herbalism/herbs/<int:herb_id>/classifications")
def api_herb_classifications(herb_id):
    """All system classifications for a herb."""
    db = get_db()
    herb = db.execute("SELECT id, name, display_name FROM alchemy_ingredients WHERE id = ?", (herb_id,)).fetchone()
    if not herb:
        return jsonify({"error": "Herb not found"}), 404

    classifications = db.execute("""
        SELECT hsc.*, ms.system_code, ms.system_name, ms.origin_region
        FROM herb_system_classifications hsc
        JOIN medicine_systems ms ON hsc.medicine_system_id = ms.id
        WHERE hsc.alchemy_ingredient_id = ?
        ORDER BY ms.system_name
    """, (herb_id,)).fetchall()

    return jsonify({"herb": dict(herb), "classifications": [dict(c) for c in classifications]})


@app.route("/api/herbalism/herbs/<int:herb_id>/monograph")
def api_herb_monograph(herb_id):
    """Full encyclopedia entry."""
    db = get_db()
    monograph = db.execute("""
        SELECT hm.*, ai.display_name, ai.icon, ai.scientific_name,
               ai.primary_effects, ai.bioactive_compounds, ai.description as herb_description
        FROM herb_monographs hm
        JOIN alchemy_ingredients ai ON hm.alchemy_ingredient_id = ai.id
        WHERE hm.alchemy_ingredient_id = ?
    """, (herb_id,)).fetchone()
    if not monograph:
        return jsonify({"error": "Monograph not found"}), 404
    return jsonify(dict(monograph))


@app.route("/api/herbalism/herbs/<int:herb_id>/history")
def api_herb_history(herb_id):
    """Historical uses timeline."""
    db = get_db()
    uses = db.execute("""
        SELECT * FROM herb_historical_uses
        WHERE alchemy_ingredient_id = ?
        ORDER BY period_start_year
    """, (herb_id,)).fetchall()
    return jsonify([dict(u) for u in uses])


@app.route("/api/herbalism/herbs/by-system/<code>")
def api_herbs_by_system(code):
    """Herbs in a tradition."""
    db = get_db()
    system = db.execute("SELECT id FROM medicine_systems WHERE system_code = ?", (code,)).fetchone()
    if not system:
        return jsonify({"error": "System not found"}), 404

    herbs = db.execute("""
        SELECT ai.*, hsc.temperature_quality, hsc.taste_qualities,
               hsc.therapeutic_actions, hsc.traditional_indications
        FROM herb_system_classifications hsc
        JOIN alchemy_ingredients ai ON hsc.alchemy_ingredient_id = ai.id
        WHERE hsc.medicine_system_id = ?
...
@app.route("/api/herbalism/methods")
def api_herbalism_methods():
    """All preparation methods."""
    db = get_db()
    methods = db.execute("SELECT * FROM herbal_preparation_methods ORDER BY method_name").fetchall()
    return jsonify([dict(m) for m in methods])


@app.route("/api/herbalism/herbs/<int:herb_id>/preparations")
def api_herb_preparations(herb_id):
    """Preparations for a herb."""
    db = get_db()
    preps = db.execute("""
        SELECT hp.*, hpm.method_code, hpm.method_name, hpm.solvent,
               hpm.duration_range, hpm.temperature_range
        FROM herb_preparations hp
        JOIN herbal_preparation_methods hpm ON hp.preparation_method_id = hpm.id
        WHERE hp.alchemy_ingredient_id = ?
        ORDER BY hpm.method_name
    """, (herb_id,)).fetchall()
    return jsonify([dict(p) for p in preps])


@app.route("/api/herbalism/prepare", methods=["POST"])
def api_herbalism_prepare():
    """Make a preparation (+XP)."""
    db = get_db()
    data = request.json
    herb_id = data.get("herb_id")
    method_id = data.get("method_id")

    if not herb_id or not method_id:
        return jsonify({"error": "herb_id and method_id required"}), 400

    # Get the preparation
    prep = db.execute("""
        SELECT hp.*, hpm.method_name, ai.display_name as herb_name
        FROM herb_preparations hp
        JOIN herbal_preparation_methods hpm ON hp.preparation_method_id = hpm.id
        JOIN alchemy_ingredients ai ON hp.alchemy_ingredient_id = ai.id
        WHERE hp.alchemy_ingredient_id = ? AND hp.preparation_method_id = ?
    """, (herb_id, method_id)).fetchone()

    if not prep:
        return jsonify({"error": "No preparation found for this herb+method combo"}), 404

    # Award XP
    xp_earned = 15
    journal = get_or_create_herbalism_journal(db)
    new_xp = journal["herbalist_xp"] + xp_earned
    new_level, new_title = get_herbalism_title(new_xp)

    db.execute("""
        UPDATE herbalism_journal SET
            herbalist_xp = ?,
            herbalist_level = ?,
            herbalist_title = ?,
            preparations_made = preparations_made + 1,
            updated_at = CURRENT_TIMESTAMP
        WHERE id = 1
...
@app.route("/api/herbalism/remedies")
def api_herbalism_remedies():
    """Browse folk remedies."""
    db = get_db()
    remedies = db.execute("""
        SELECT fr.*, ms.system_name, ms.system_code,
               hpm.method_name as prep_method_name
        FROM folk_remedies fr
        LEFT JOIN medicine_systems ms ON fr.medicine_system_id = ms.id
        LEFT JOIN herbal_preparation_methods hpm ON fr.preparation_method_id = hpm.id
        ORDER BY fr.remedy_name
    """).fetchall()
    return jsonify([dict(r) for r in remedies])


@app.route("/api/herbalism/remedies/<int:remedy_id>")
def api_herbalism_remedy_detail(remedy_id):
    """Remedy with full details."""
    db = get_db()
    remedy = db.execute("""
        SELECT fr.*, ms.system_name, ms.system_code, ms.core_philosophy,
               hpm.method_name as prep_method_name, hpm.general_instructions
        FROM folk_remedies fr
        LEFT JOIN medicine_systems ms ON fr.medicine_system_id = ms.id
        LEFT JOIN herbal_preparation_methods hpm ON fr.preparation_method_id = hpm.id
        WHERE fr.id = ?
    ...
    return jsonify(dict(remedy))


@app.route("/api/herbalism/remedies/<int:remedy_id>/make", methods=["POST"])
def api_herbalism_make_remedy(remedy_id):
    """Make a remedy (+XP)."""
    db = get_db()
    remedy = db.execute("SELECT * FROM folk_remedies WHERE id = ?", (remedy_id,)).fetchone()
    if not remedy:
        return jsonify({"error": "Remedy not found"}), 404

    xp_earned = remedy["discovery_xp"] or 25
    journal = get_or_create_herbalism_journal(db)

    # Track discovered remedies
    discovered = json.loads(journal["remedies_discovered"] or "[]")
    is_new = remedy_id not in discovered
    if is_new:
        discovered.append(remedy_id)
        xp_earned += 10  # bonus for first discovery
    ...
    db.execute("""
        UPDATE herbalism_journal SET
            ...
            updated_at = CURRENT_TIMESTAMP
        WHERE id = 1
    """, (new_xp, new_level, new_title, json.dumps(discovered)))
    db.commit()
...
@app.route("/api/herbalism/herbs/<int:herb_id>/interactions")
def api_herb_interactions(herb_id):
    """Drug interactions for a herb."""
    db = get_db()
    interactions = db.execute("""
        SELECT * FROM herb_drug_interactions
        WHERE alchemy_ingredient_id = ?
        ORDER BY severity DESC, drug_name
    """, (herb_id,)).fetchall()
    return jsonify([dict(i) for i in interactions])


@app.route("/api/herbalism/interactions/check", methods=["POST"])
def api_herbalism_check_interactions():
    """Check herbs vs medications."""
    db = get_db()
    data = request.json
    ...
    if not herb_ids:
        return jsonify({"error": "herb_ids required"}), 400
    ...
    if medications:
        for med in medications:
            interactions = db.execute(f"""
                SELECT hdi.*, ai.display_name as herb_name
                FROM herb_drug_interactions hdi
                JOIN alchemy_ingredients ai ON hdi.alchemy_ingredient_id = ai.id
                WHERE hdi.alchemy_ingredient_id IN ({placeholders})
                AND (hdi.drug_name LIKE ? OR hdi.drug_class LIKE ?)
                ORDER BY hdi.severity DESC
            """, herb_ids + [f"%{med}%", f"%{med}%"]).fetchall()
            results.extend([dict(i) for i in interactions])
    else:
        interactions = db.execute(f"""
            SELECT hdi.*, ai.display_name as herb_name
            FROM herb_drug_interactions hdi
            ...
            ORDER BY hdi.severity DESC
        """, herb_ids).fetchall()
        results = [dict(i) for i in interactions]

    return jsonify({"interactions": results, "has_severe": any(r["severity"] == "high" for r in results)})


@app.route("/api/herbalism/herbs/<int:herb_id>/culinary-history")
def api_herb_culinary_history(herb_id):
    """Herb across food traditions."""
    db = get_db()
    traditions = db.execute("""
        SELECT * FROM herb_culinary_traditions
        WHERE alchemy_ingredient_id = ?
        ORDER BY cuisine_tradition
    """, (herb_id,)).fetchall()
    return jsonify([dict(t) for t in traditions])


@app.route("/api/herbalism/journal")
def api_herbalism_journal():
    """Herbalism progress/XP."""
    db = get_db()
    journal = get_or_create_herbalism_journal(db)
    result = dict(journal)
    result["levels"] = [{"threshold": t, "title": title} for t, title in HERBALISM_LEVELS]
    result["next_level_xp"] = None
    for t, title in HERBALISM_LEVELS:
        if t > journal["herbalist_xp"]:
            result["next_level_xp"] = t
            break
    return jsonify(result)


@app.route("/api/herbalism/journal/explore-system", methods=["POST"])
def api_herbalism_explore_system():
    """Log exploring a system (+XP)."""
    db = get_db()
    data = request.json
    system_code = data.get("system_code")

    if not system_code:
        return jsonify({"error": "system_code required"}), 400

    system = db.execute("SELECT * FROM medicine_systems WHERE system_code = ?", (system_code,)).fetchone()
    if not system:
        return jsonify({"error": "System not found"}), 404

    journal = get_or_create_herbalism_journal(db)
    explored = json.loads(journal["systems_explored"] or "[]")

    is_new = system_code not in explored
    xp_earned = 20 if is_new else 5

    if is_new:
        explored.append(system_code)
    ...
    db.execute("""
        UPDATE herbalism_journal SET
            ...
            updated_at = CURRENT_TIMESTAMP
        WHERE id = 1
    """, (new_xp, new_level, new_title, json.dumps(explored)))
    db.commit()
...
# ============================================================================
# FOOD CATALOG SEMANTIC SEARCH API
# ============================================================================

@app.route("/api/foods/search")
def api_foods_semantic_search():
    """Semantic search across the food catalog using tags.

    Query params:
        q          - free text search (matches name, description)
        cuisine    - filter by cuisine (e.g. Japanese, Italian)
        meal_type  - filter by meal type (breakfast, lunch, dinner, snack, dessert)
        cooking_method - filter (fried, baked, grilled, steamed, raw, fermented...)
        flavor     - filter by flavor profile (sweet, spicy, savory, sour...)
        dietary    - filter (vegetarian, vegan, gluten-free, halal, kosher)
        season     - filter (summer, winter, spring, autumn)
        tradition  - filter (Christmas, Easter, Ramadan, wedding...)
        course     - filter (soup, salad, bread, pasta, rice dish, sandwich...)
        temperature - filter (hot, cold, warm)
        texture    - filter (crispy, creamy, chewy, soft...)
        region     - filter (East Asia, Mediterranean, South America...)
        ingredient - filter by ingredient
        food_type  - filter (fruit, vegetable, meat, seafood, dairy, grain, spice)
        occasion   - filter (party, picnic, comfort food, formal...)
        historical_period - filter (ancient, medieval, modern, pre-Columbian...)
        trade_route - filter (Silk Road, Spice Route, Columbian Exchange...)
        cooking_science - filter (Maillard reaction, fermentation, emulsion...)
        origin_hemisphere - filter (Eastern, Western, Northern, Southern)
        columbian_exchange - filter (Old World, New World)
        staple_food - filter (yes, no, primary, secondary)
        cultural_significance - filter (sacred, celebratory, everyday...)
        social_class - filter (aristocratic, peasant, universal...)
        tags       - comma-separated tag values (searches across all categories)
        limit      - results per page (default 50)
        offset     - pagination offset
        sort       - name, random (default: relevance)
    """
    db = get_db()
    ...
    # Tag filters
    tag_filters = {}
    for param in ["cuisine", "meal_type", "cooking_method", "flavor_profile",
                   "dietary", "season", "tradition", "course", "temperature",
                   "texture", "region", "ingredient", "food_type", "occasion",
                   "time_of_day", "historical_period", "trade_route",
                   "cooking_science", "origin_hemisphere", "columbian_exchange",
                   "staple_food", "cultural_significance", "social_class"]:
        val = request.args.get(param, "")
        if val:
            # Map shorthand param names
            cat = param
            if param == "flavor":
                cat = "flavor_profile"
            tag_filters[cat] = val

    # Also support flavor= shorthand
    if request.args.get("flavor"):
        tag_filters["flavor_profile"] = request.args.get("flavor")

    # Generic tags param (comma-separated, searches all categories)
    generic_tags = [t.strip() for t in request.args.get("tags", "").split(",") if t.strip()]
    ...
    if q:
        conditions.append("(f.name LIKE ? OR f.description LIKE ?)")
        params.extend([f"%{q}%", f"%{q}%"])

    # Each tag filter requires a subquery
    for i, (cat, val) in enumerate(tag_filters.items()):
        conditions.append(f"""f.id IN (
            SELECT food_id FROM food_tags
            WHERE tag_category = ? AND tag_value LIKE ?
        )""")
        params.extend([cat, f"%{val}%"])

    # Generic tags - match any category
    for tag in generic_tags:
        conditions.append(f"""f.id IN (
            SELECT food_id FROM food_tags WHERE tag_value LIKE ?
        )""")
        params.append(f"%{tag}%")
    ...
    # Count total
    count_sql = f"SELECT COUNT(*) as cnt FROM foods f WHERE {where}"
    ...
    if sort == "random":
        ...
        # Relevance: prefer foods with more tags and descriptions
        order = "CASE WHEN f.description IS NOT NULL THEN 0 ELSE 1 END, f.name ASC"

    sql = f"""
        SELECT f.id, f.name, f.slug, f.food_type, f.primary_category,
               f.wikipedia_url, f.wikidata_qid, f.image_url,
               f.is_vegetarian, f.is_vegan, f.is_gluten_free,
               f.description, f.data_source
        FROM foods f
        WHERE {where}
        ORDER BY {order}
        LIMIT ? OFFSET ?
    ...
    for row in rows:
        food = dict(row)
        tags_rows = db.execute(
            "SELECT tag_category, tag_value FROM food_tags WHERE food_id = ? ORDER BY tag_category",
            (food["id"],)
        ).fetchall()
        tags = {}
        for t in tags_rows:
            tags.setdefault(t["tag_category"], []).append(t["tag_value"])
        food["tags"] = tags
        results.append(food)
...
def api_food_detail_by_id(food_id):
    """Get full detail for a single food including all tags, ingredients, pairings."""
    db = get_db()
    ...
    # Ingredients
    ingredients = db.execute(
        "SELECT ingredient_name, wikidata_qid FROM food_ingredients_wiki WHERE food_id = ?",
        (food_id,)
    ).fetchall()
    ...
    # Pairings
    pairings = db.execute(
        "SELECT paired_food_name, pairing_type FROM food_pairings WHERE food_id = ?",
        (food_id,)
    ).fetchall()
    result["pairings"] = [dict(p) for p in pairings]

    # Culture origins
    origins = db.execute("""
        SELECT c.name as culture, c.region, c.culture_type, fco.origin_type
        FROM food_culture_origins fco
        JOIN cultures c ON c.id = fco.culture_id
        WHERE fco.food_id = ?
    """, (food_id,)).fetchall()
    result["origins"] = [dict(o) for o in origins]

    return jsonify(result)


@app.route("/api/foods/tags")
def api_food_tags_overview():
    """Get all available tag categories and their values with counts."""
    db = get_db()
    category = request.args.get("category", "")
    ...
    if category:
        rows = db.execute("""
            ...
            FROM food_tags WHERE tag_category = ?
            GROUP BY tag_value ORDER BY count DESC
        """, (category,)).fetchall()
        return jsonify({"category": category, "values": [dict(r) for r in rows]})

    # All categories summary
    rows = db.execute("""
        SELECT tag_category, COUNT(DISTINCT tag_value) as unique_values,
               COUNT(*) as total_tags, COUNT(DISTINCT food_id) as foods_tagged
        FROM food_tags GROUP BY tag_category ORDER BY total_tags DESC
    """).fetchall()
    return jsonify({"categories": [dict(r) for r in rows]})


@app.route("/api/foods/discover")
def api_foods_discover():
    """Discover foods by combining multiple tag dimensions.

    Returns foods grouped by a chosen dimension, useful for browsing.
    Query params:
        group_by  - tag category to group results by (default: cuisine)
        filter_*  - any tag filter (same as /api/foods/search)
        limit     - results per group (default 5)
    """
    db = get_db()
    group_by = request.args.get("group_by", "cuisine")
    per_group = min(int(request.args.get("limit", 5)), 20)

    # Get top values for the group_by category
    top_groups = db.execute("""
        SELECT tag_value, COUNT(*) as cnt
        FROM food_tags WHERE tag_category = ?
        GROUP BY tag_value ORDER BY cnt DESC LIMIT 20
    """, (group_by,)).fetchall()

    result = {}
    for group in top_groups:
        group_val = group["tag_value"]
        foods = db.execute("""
            SELECT f.id, f.name, f.slug, f.image_url, f.description, f.food_type
            FROM foods f
            JOIN food_tags ft ON ft.food_id = f.id
            WHERE ft.tag_category = ? AND ft.tag_value = ?
            AND f.description IS NOT NULL
            ORDER BY RANDOM()
            LIMIT ?
        """, (group_by, group_val, per_group)).fetchall()
        if foods:
            result[group_val] = {
                "count": group["cnt"],
                "sample": [dict(f) for f in foods]
            }

    return jsonify({"group_by": group_by, "groups": result})


@app.route("/api/foods/similar/<int:food_id>")
def api_foods_similar(food_id):
    """Find foods similar to a given food based on shared tags."""
    db = get_db()
    limit = min(int(request.args.get("limit", 10)), 50)

    # Get this food's tags
    my_tags = db.execute(
        "SELECT tag_category, tag_value FROM food_tags WHERE food_id = ?", (food_id,)
    ).fetchall()

    if not my_tags:
        return jsonify({"error": "Food has no tags"}), 404

    # Find foods with the most overlapping tags
    placeholders = ",".join(["(?, ?)"] * len(my_tags))
    params = []
    for t in my_tags:
        params.extend([t["tag_category"], t["tag_value"]])

    similar = db.execute(f"""
        SELECT ft.food_id, f.name, f.slug, f.image_url, f.description,
               COUNT(*) as shared_tags
        FROM food_tags ft
        JOIN foods f ON f.id = ft.food_id
        WHERE (ft.tag_category, ft.tag_value) IN (VALUES {placeholders})
        AND ft.food_id != ?
        GROUP BY ft.food_id
        ORDER BY shared_tags DESC
        LIMIT ?
    """, params + [food_id, limit]).fetchall()
...
# ============================================================================
# ANTHROPOLOGICAL / CIVILIZATIONS API
# ============================================================================


@app.route("/api/civilizations")
def api_civilizations():
    """Get all civilizations with era counts and food counts."""
    db = get_db()
    rows = db.execute("""
        SELECT c.id, c.name, c.slug, c.culture_type, c.region, c.subregion,
               c.modern_countries, c.era_start_year, c.era_end_year,
               c.is_living_culture, c.description,
               (SELECT COUNT(*) FROM culture_eras WHERE culture_id = c.id) as era_count,
...
@app.route("/api/civilizations/<slug>")
def api_civilization_detail(slug):
    """Full civilization detail with eras, meal patterns, and top foods."""
    db = get_db()
    culture = db.execute("SELECT * FROM cultures WHERE slug = ?", (slug,)).fetchone()
    if not culture:
    ...
    # Historical eras
    eras = db.execute("""
        SELECT * FROM culture_eras WHERE culture_id = ? ORDER BY start_year
    """, (culture["id"],)).fetchall()
    result["eras"] = [dict(e) for e in eras]
    ...
    """, (culture["id"],)).fetchall()
    result["meal_patterns"] = [dict(p) for p in patterns]

    # Top foods (with images preferred)
    foods = db.execute("""
        SELECT f.id, f.name, f.slug, f.image_url, f.description, f.food_type
        FROM foods f
        JOIN food_culture_origins fco ON f.id = fco.food_id
        ...
        LIMIT 50
    """, (culture["id"],)).fetchall()
    result["foods"] = [dict(f) for f in foods]

    return jsonify(result)


@app.route("/api/civilizations/<slug>/timeline")
def api_civilization_timeline(slug):
    """Timeline of a civilization's food history with tagged foods per era."""
    db = get_db()
    culture = db.execute("SELECT id, name FROM cultures WHERE slug = ?", (slug,)).fetchone()
    if not culture:
        return jsonify({"error": "Civilization not found"}), 404
    ...
    for era in eras:
        era_dict = dict(era)
        # Find foods that match this historical period
        period_keywords = era["era_name"].lower().split()
        era_dict["sample_foods"] = []
        timeline.append(era_dict)

    return jsonify({"culture": dict(culture), "timeline": timeline})


@app.route("/api/food-science")
def api_food_science():
    """Browse food science categories and their foods."""
    db = get_db()
    category = request.args.get("category", "")

    if category:
        foods = db.execute("""
            SELECT f.id, f.name, f.slug, f.image_url, f.description
            FROM foods f
            JOIN food_tags ft ON ft.food_id = f.id
            WHERE ft.tag_category = 'cooking_science' AND ft.tag_value = ?
            ORDER BY f.name
            LIMIT 100
        """, (category,)).fetchall()
        return jsonify({"category": category, "foods": [dict(f) for f in foods]})

    # Summary of all cooking science tags
    rows = db.execute("""
        SELECT tag_value as category, COUNT(DISTINCT food_id) as food_count
        FROM food_tags WHERE tag_category = 'cooking_science'
        GROUP BY tag_value ORDER BY food_count DESC
    """).fetchall()
    return jsonify({"categories": [dict(r) for r in rows]})
...
def api_trade_routes():
    """Browse trade routes and their associated foods."""
    db = get_db()
    ...
    if route:
        foods = db.execute("""
            ...
            FROM foods f
            JOIN food_tags ft ON ft.food_id = f.id
            WHERE ft.tag_category = 'trade_route' AND ft.tag_value = ?
            ORDER BY f.name
            LIMIT 100
        """, (route,)).fetchall()
        return jsonify({"route": route, "foods": [dict(f) for f in foods]})

    rows = db.execute("""
        SELECT tag_value as route, COUNT(DISTINCT food_id) as food_count
        FROM food_tags WHERE tag_category = 'trade_route'
        GROUP BY tag_value ORDER BY food_count DESC
    """).fetchall()
    return jsonify({"routes": [dict(r) for r in rows]})


@app.route("/api/columbian-exchange")
def api_columbian_exchange():
    """Browse foods by Old World / New World origin."""
    db = get_db()
    ...
    if hemisphere:
        foods = db.execute("""
            ...
            FROM foods f
            JOIN food_tags ft ON ft.food_id = f.id
            WHERE ft.tag_category = 'origin_hemisphere' AND ft.tag_value LIKE ?
            ORDER BY f.name
            LIMIT ?
        """, (f"%{hemisphere}%", limit)).fetchall()
        return jsonify({"hemisphere": hemisphere, "foods": [dict(f) for f in foods]})

    rows = db.execute("""
        SELECT tag_value as hemisphere, COUNT(DISTINCT food_id) as food_count
        FROM food_tags WHERE tag_category = 'origin_hemisphere'
        GROUP BY tag_value ORDER BY food_count DESC
    """).fetchall()

    exchange_foods = db.execute("""
        SELECT COUNT(DISTINCT food_id) as count
        FROM food_tags WHERE tag_category = 'columbian_exchange'
    """).fetchone()

    return jsonify({
        "hemispheres": [dict(r) for r in rows],
        "columbian_exchange_foods": exchange_foods["count"]
    })


@app.route("/api/social-history")
def api_social_history():
    """Browse foods by historical social class."""
    db = get_db()
    social_class = request.args.get("class", "")
    ...
    if social_class:
        foods = db.execute("""
            ...
            FROM foods f
            JOIN food_tags ft ON ft.food_id = f.id
            WHERE ft.tag_category = 'social_class' AND ft.tag_value = ?
            ORDER BY f.name
            LIMIT 100
        """, (social_class,)).fetchall()
        return jsonify({"class": social_class, "foods": [dict(f) for f in foods]})

    rows = db.execute("""
        SELECT tag_value as social_class, COUNT(DISTINCT food_id) as food_count
        FROM food_tags WHERE tag_category = 'social_class'
        GROUP BY tag_value ORDER BY food_count DESC
    """).fetchall()
    return jsonify({"classes": [dict(r) for r in rows]})


@app.route("/api/food-history/timeline")
def api_food_history_timeline():
    """Global food history timeline showing major periods and their foods."""
    db = get_db()

    periods = db.execute("""
        SELECT tag_value as period, COUNT(DISTINCT food_id) as food_count
        FROM food_tags WHERE tag_category = 'historical_period'
        GROUP BY tag_value ORDER BY food_count DESC
    """).fetchall()
    ...
    for period in periods:
        sample = db.execute("""
            SELECT f.id, f.name, f.slug, f.image_url
            FROM foods f
            JOIN food_tags ft ON ft.food_id = f.id
            WHERE ft.tag_category = 'historical_period' AND ft.tag_value = ?
            ORDER BY RANDOM() LIMIT 5
        """, (period["period"],)).fetchall()
        result.append({
            "period": period["period"],
            "food_count": period["food_count"],
            "sample_foods": [dict(s) for s in sample]
        })
...
def api_database_stats():
    ...
    stats = {}
    stats["total_foods"] = db.execute("SELECT COUNT(*) as cnt FROM foods").fetchone()["cnt"]
    stats["foods_with_descriptions"] = db.execute(
        "SELECT COUNT(*) as cnt FROM foods WHERE description IS NOT NULL AND description != ''"
    ).fetchone()["cnt"]
    stats["foods_with_images"] = db.execute(
        "SELECT COUNT(*) as cnt FROM foods WHERE image_url IS NOT NULL AND image_url != ''"
    ).fetchone()["cnt"]
    stats["total_tags"] = db.execute("SELECT COUNT(*) as cnt FROM food_tags").fetchone()["cnt"]
    stats["total_cultures"] = db.execute(
        "SELECT COUNT(*) as cnt FROM cultures WHERE culture_type = 'civilization'"
    ).fetchone()["cnt"]
    stats["total_eras"] = db.execute("SELECT COUNT(*) as cnt FROM culture_eras").fetchone()["cnt"]
    stats["total_meal_patterns"] = db.execute(
        "SELECT COUNT(*) as cnt FROM culture_meal_patterns"
    ).fetchone()["cnt"]
    stats["food_culture_links"] = db.execute(
        "SELECT COUNT(*) as cnt FROM food_culture_origins"
    ).fetchone()["cnt"]

    # Tag categories breakdown
    tag_cats = db.execute("""
        SELECT tag_category, COUNT(*) as tag_count, COUNT(DISTINCT food_id) as food_count
        FROM food_tags GROUP BY tag_category ORDER BY tag_count DESC
    """).fetchall()
    stats["tag_categories"] = [dict(t) for t in tag_cats]

    # Recipe counts
    stats["recipes_small"] = db.execute("SELECT COUNT(*) as cnt FROM recipes").fetchone()["cnt"]
    stats["recipes_large"] = db.execute("SELECT COUNT(*) as cnt FROM recipes_large").fetchone()["cnt"]

    return jsonify(stats)


# ============================================================================
# WIKIPEDIA IMPORT ADMIN API
# ============================================================================

_import_status = {"running": False, "progress": 0, "total": 0, "message": "", "errors": []}


@app.route("/api/import/wikipedia-foods", methods=["POST"])
def api_import_wikipedia_foods():
    """Trigger Wikidata import (runs import_wikipedia_foods.py)."""
    import subprocess
    import threading
    ...
    def run_import():
        ...
        _import_status["message"] = "Starting Wikidata SPARQL import..."
        try:
            script = os.path.join(os.path.dirname(__file__), "import_wikipedia_foods.py")
            result = subprocess.run(
                [sys.executable, script, "--mode", "wikidata"],
                capture_output=True, text=True, timeout=600
            )
            _import_status["message"] = f"Complete: {result.stdout[-200:]}" if result.stdout else "Done"
            if result.returncode != 0:
                _import_status["errors"].append(result.stderr[-500:] if result.stderr else "Unknown error")
        except Exception as e:
            _import_status["errors"].append(str(e))
        finally:
...
@app.route("/api/import/wikipedia-categories", methods=["POST"])
def api_import_wikipedia_categories():
    """Trigger category tree walk."""
    import subprocess
    import threading
    ...
    def run_import():
        _import_status["running"] = True
        _import_status["message"] = "Starting Wikipedia category walker..."
        try:
            script = os.path.join(os.path.dirname(__file__), "import_wikipedia_foods.py")
            result = subprocess.run(
                [sys.executable, script, "--mode", "categories"],
                capture_output=True, text=True, timeout=600
            )
            _import_status["message"] = f"Complete: {result.stdout[-200:]}" if result.stdout else "Done"
            if result.returncode != 0:
    ...
    threading.Thread(target=run_import, daemon=True).start()
    return jsonify({"success": True, "message": "Category import started"})
...
def api_import_status():
    """Import progress."""
    db = get_db()
    food_count = db.execute("SELECT COUNT(*) as cnt FROM foods").fetchone()["cnt"]
    culture_count = db.execute("SELECT COUNT(*) as cnt FROM cultures").fetchone()["cnt"]
    recipe_count = db.execute("SELECT COUNT(*) as cnt FROM anthropological_recipes").fetchone()["cnt"]
