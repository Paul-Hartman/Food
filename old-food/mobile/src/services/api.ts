...
class ApiService {
  ...
  /**
   * Get all saved scale containers (tare weights)
   */
  async getScaleContainers(): Promise<Array<{
    id: number;
    name: string;
    ...
    return this.fetchJson('/api/scale/containers');
  }

  /**
   * Add new scale container
   */
  async addScaleContainer(data: {
    name: string;
    tare_weight_g: number;
    color_hex?: string;
    icon_emoji?: string;
    notes?: string;
  }): Promise<{ success: boolean; container_id: number }> {
    return this.fetchJson('/api/scale/containers', {
      method: 'POST',
      body: JSON.stringify(data),
    });
   ...
   */
  async logScaleMeasurement(data: {
    product_id?: number;
    pantry_inventory_id?: number;
    ...
    recipe_id?: number;
    notes?: string;
  }): Promise<{ success: boolean; measurement_id: number }> {
    return this.fetchJson('/api/scale/measure', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
   ...
   */
  async getScaleMeasurements(params?: {
    product_id?: number;
    limit?: number;
  }): Promise<Array<{
    id: number;
    ...
    return this.fetchJson(`/api/scale/measurements${query}`);
  }

  /**
   * Update pantry inventory with scale measurement
   */
  async weighPantryItem(inventoryId: number, data: {
    net_weight_g: number;
    tare_weight_g?: number;
    save_tare?: boolean;
  }): Promise<{ success: boolean; new_weight: number }> {
    return this.fetchJson(`/api/pantry/inventory/${inventoryId}/weigh`, {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
  ...
  /**
   * Get all active infusions
   */
  async getInfusions(status?: 'active' | 'completed'): Promise<Array<{
    id: number;
    name: string;
    type: string;
    start_date: string;
    ...
    return this.fetchJson(`/api/scale/infusions${query}`);
  }

  /**
   * Create new infusion tracking
   */
  ...
  }): Promise<{ success: boolean; infusion_id: number }> {
    return this.fetchJson('/api/scale/infusions', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
   ...
   */
  async logInfusionCheckIn(infusionId: number, data: {
    weight_g: number;
    notes?: string;
  }): Promise<{ success: boolean }> {
    return this.fetchJson(`/api/scale/infusions/${infusionId}/checkin`, {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
   ...
   */
  async getInfusionCheckIns(infusionId: number): Promise<Array<{
    id: number;
    day_number: number;
    weight_g: number;
    notes?: string;
    ...
    return this.fetchJson(`/api/scale/infusions/${infusionId}/checkins`);
  }

  /**
   * Get coffee brew logs
   */
  async getBrewLogs(params?: {
    method?: string;
    limit?: number;
  }): Promise<Array<{
    id: number;
    ...
    const query = queryParams.toString() ? `?${queryParams}` : '';
    return this.fetchJson(`/api/scale/brew-logs${query}`);
  }

  /**
   * Log coffee brew session
   */
  async logBrewSession(data: {
    brew_method: string;
    coffee_weight_g: number;
    water_weight_g: number;
    ratio?: number;
    ...
    rating?: number;
    notes?: string;
  }): Promise<{ success: boolean; brew_id: number }> {
    return this.fetchJson('/api/scale/brew-logs', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  }
