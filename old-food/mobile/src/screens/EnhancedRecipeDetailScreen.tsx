...
export default function EnhancedRecipeDetailScreen() {
  ...
  const [targetIngredientAmount, setTargetIngredientAmount] = useState<string>('');
  ...
  // Calculate multiplier based on scaling mode
  let multiplier = currentServings / r.servings; // Default: serving-based

  if (scalingMode === 'ingredient' && selectedIngredientIndex !== null && targetIngredientAmount) {
    const selectedIng = ingredients[selectedIngredientIndex];
    const originalAmount = parseFloat(String(selectedIng.quantity)) || 0;
    const targetAmount = parseFloat(targetIngredientAmount) || 0;
    if (originalAmount > 0 && targetAmount > 0) {
      multiplier = targetAmount / originalAmount;
    }
  }

  const servingMultiplier = multiplier;
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        {/* Ingredient-Based Scaling Card */}
        <View style={styles.scalingCard}>
          <View style={styles.scalingModeSelector}>
            <TouchableOpacity
              style={[styles.modeButton, scalingMode === 'servings' && styles.modeButtonActive]}
              onPress={() => setScalingMode('servings')}
            >
              <Text style={[styles.modeButtonText, scalingMode === 'servings' && styles.modeButtonTextActive]}>
                Scale by Servings
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.modeButton, scalingMode === 'ingredient' && styles.modeButtonActive]}
              onPress={() => setScalingMode('ingredient')}
            >
              <Text style={[styles.modeButtonText, scalingMode === 'ingredient' && styles.modeButtonTextActive]}>
                Scale by Ingredient
              </Text>
            </TouchableOpacity>
          </View>
          ...
          {scalingMode === 'ingredient' && (
            <View style={styles.ingredientScalerContent}>
              <Text style={styles.scalerLabel}>I have this much of:</Text>
              <TouchableOpacity
                style={styles.ingredientPickerButton}
                onPress={() => setShowIngredientPicker(true)}
              >
                <Text style={styles.ingredientPickerText}>
                  {selectedIngredientIndex !== null
                    ? ingredients[selectedIngredientIndex].name
                    : 'Select an ingredient'}
                </Text>
                <Text style={styles.ingredientPickerIcon}>▼</Text>
              </TouchableOpacity>
              {selectedIngredientIndex !== null && (
                <View style={styles.amountInputRow}>
                  <TextInput
                    ...
                    placeholder="Amount"
                    value={targetIngredientAmount}
                    onChangeText={setTargetIngredientAmount}
                  />
                  <Text style={styles.amountUnit}>
                    {ingredients[selectedIngredientIndex].unit}
                  </Text>
                  <Text style={styles.amountOriginal}>
                    (Recipe: {ingredients[selectedIngredientIndex].quantity} {ingredients[selectedIngredientIndex].unit})
                  </Text>
                </View>
              )}
              {multiplier !== 1 && (
                <Text style={styles.scalingInfo}>
                  {multiplier > 1 ? '📈' : '📉'} Scaling {multiplier < 1 ? 'down' : 'up'} to {(multiplier * 100).toFixed(0)}%
                </Text>
              )}
            </View>
          )}
              ...
              {ingredients.map((ing, index) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    selectedIngredientIndex === index && styles.ingredientListItemSelected
                  ]}
                  onPress={() => {
                    setSelectedIngredientIndex(index);
                    setTargetIngredientAmount(String(ing.quantity));
                    setShowIngredientPicker(false);
                  }}
                >
                  <Text style={styles.ingredientListItemName}>{ing.name}</Text>
                  <Text style={styles.ingredientListItemAmount}>
