...
1. **AltServer** installed (https://altstore.io)
1. **Apple ID** (free account works)

### On Your iPhone:

1. **AltStore** installed (follow https://altstore.io installation guide)
1. Same WiFi network as your Mac
...
1. Click **Generate an app-specific password**
1. Name it: "Food App Build"
1. Copy the password (format: `xxxx-xxxx-xxxx-xxxx`)
1. Save it - you'll need this when building
...
1. Open **Finder**
1. Select your iPhone in sidebar
1. Drag the `.ipa` file to the iPhone
1. Open **AltStore** on iPhone
1. Tap **My Apps**
1. Find "Food App" and tap **Install**
...
1. Tap **Trust "Your Name"**
1. Confirm **Trust**
1. Return to home screen and launch Food app
...
1. Set date to 1-2 days in the past
1. Launch the app
1. Turn **Set Automatically** back ON
...
1. Keep iPhone on same WiFi
1. AltStore will auto-refresh in the background
...
1. **API URL:** App is configured to connect to `http://192.168.2.38:5025`
1. **Update API URL:** If your Mac has a different IP, update in `mobile/app.json`:
   ```json
   "extra": {
     "apiUrl": "http://YOUR_MAC_IP:5025"
   }
   ```
1. **Rebuild** if you change the API URL
